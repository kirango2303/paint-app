...
(** Helper function: iter f into 'a list *)		
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =	
  ...
  let widget: widget = {
    repaint = (fun (g: Gctx.gctx) -> 
        let (width, height) = w.size () in
        (if control.get_value () then (* Draw the cross *)	
           (
             Gctx.draw_line g (0,0) (height,height);
             Gctx.draw_line g (0,height) (height,0)
           )
         else ()); 
        Gctx.draw_rect g (0,0) (height,height); (* Draw the box *)
        w.repaint (Gctx.translate g (height+10,0)))
        (* Move label to the right of the box *)
    ;
    handle = (mouseclick_listener 
                (fun () -> control.change_value (not (control.get_value ()))));
        (* Change the current value to the opposite *)
    size = (fun () -> let (x,y) = w.size () in
             ((y + x+10), y ) )
        (* Combination of label size and box size *)
  } in

  border widget, control
...
let slider (num: int) (s: string) : widget * int value_controller =	
  let (w, lc) = label s in
  let control = make_controller num in
  let w1: widget = {
  repaint = (fun (g: Gctx.gctx) -> (* Make a rect and a filled rect *)
        let (width, height) = w.size () in
        let v = control.get_value () in 
        Gctx.fill_rect g (0,0) (v, height); 
        Gctx.draw_rect g (0,0) (100, height));
	handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
  (* Change the value to the current position of mouse click/ drag *)
	let pos = Gctx.event_pos e g in 
  (if (Gctx.event_type e = Gctx.MouseDrag ||
       Gctx.event_type e = Gctx.MouseDown) 
	then control.change_value (fst pos) else () ));
	size = (fun () -> let (x,y) = w.size () in
             (100, y ) )	} in	(* Size of the outer rect *)
	
	let widget = hpair w w1 in (* Pairing the slider with the label *)
  border widget, control	 
